...
CI автоматически запускается при каждом `push` и `pull request`.

Пайплайн выполняет следующие действия:

1. Загружает репозиторий.
2. Устанавливает инструменты сборки.
3. Собирает debug-версию проекта: -fsanitize=address,undefined
4. Запускает тесты с использованием ASAN_OPTIONS=detect_leaks=1

При запуске тестов используются:

* AddressSanitizer (ASan) — для обнаружения ошибок управления памятью;
* LeakSanitizer (LSan) — для обнаружения утечек памяти;
* UndefinedBehaviorSanitizer (UBSan) — для обнаружения неопределённого поведения.
...
Успешный запуск CI подтверждает, что проект корректно собирается, проходит тесты и не содержит обнаруживаемых ошибок управления памятью в покрываемых сценариях.
